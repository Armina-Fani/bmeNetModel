...
from varunModelCode import BMENet
...
class ClientCreator:
    ...
    def mycollate_full(self, x):
        return amcollate(x, labelname=self.label)
...
def amcollate(mlist, labelname="sublabel", inputsize=(256, 256, 256), labelsize=(256, 256, 256)):
    ...
    if num_subjs > 1:
        ...
        labels = torch.empty(num_subjs, *labelsize, requires_grad=False, dtype=torch.long)
        ...
        label = torch.vstack([torch.from_numpy(np.frombuffer(sub[labelname], dtype=np.uint8)).long() for sub in mdict[subj]])
...
def amcollate2(mlist, labelname="sublabel"):
    def preprocess_image(img):
    ...
    if num_subjs > 1:
        ...
        labels = torch.empty(
            num_subjs, *labelsize, requires_grad=False, dtype=torch.long
            )
        for i, subj in enumerate(mdict):
            mdict[subj].sort(key=lambda _: _["id"])
            cube = np.vstack([sub["subdata"] for sub in mdict[subj]])
